...
import { gsap } from 'gsap'
import { useHover } from 'reev/src/gesture/hover/react'
...
```jsx
import { Hover } from 'reev/gesture/hover/react'

export const hover = ({ active, target, movement: [x, y] }) => {
        gsap.to(target, { x, y, scale: active ? 1.2 : 1 })
}

export const App2 = () => {
        return <Hover hover={hover}>{({ ref }) => <span ref={ref} style={style} />}</Hover>
}
```

import { Hover } from 'reev/src/gesture/hover/react'

# Core API for React

## useMutable

```tsx
import { useState } from 'react'
import { useMutable } from 'reev/react'

export const App = () => {
        const [i, set] = useState(0)
        const { click } = useMutable({ click: () => set(i + 1) })
        return <button onClick={click}>{i}</button>
}
```

import { useState } from 'react'
import { useMutable } from '../../../packages/core/react'

export const App = () => {
        const [i, set] = useState(0)
        const { click } = useMutable({ click: () => set(i + 1) })
        return (
                <button onClick={click} className="clean-btn button button--secondary margin-left--md">
                        {i}
                </button>
        )
}

<App />

## useEvent

```tsx
import { useState } from 'react'
import { useEvent } from 'reev/react'

export const App2 = () => {
        const [i, set] = useState(0)
        const e = useEvent({ click() {} }) // do something when component will be clicked
        e('click', () => set((p) => p + 1)) // register new function
        return <button onClick={e.click}>{i}</button>
}
```

import { useEvent } from '../../../packages/core/react'

export const App2 = () => {
        const [i, set] = useState(0)
        const event = useEvent({ click() {} }) // do something when component will be clicked
        event('click', () => set((p) => p + 1)) // register new function
        return (
                <button onClick={event.click} className="clean-btn button button--secondary margin-left--md">
                        {i}
                </button>
        )
}

<App2 />

## useRefEvent

```tsx
import { useRefEvent } from 'reev/react'

export const App3 = () => {
        const [i, set] = useState(0)
        const event = useRefEvent({
                mount() {}, // do something when component did mount
                clean() {}, // do something when component did unmount
                click() {}, // do something when component will be clicked
        })
        event('click', () => set((p) => p + 1)) // register new function
        return (
                <button ref={event.ref} onClick={event.click}>
                        {i}
                </button>
        )
}
```

import { useRefEvent } from '../../../packages/core/react'

export const App3 = () => {
        const [i, set] = useState(0)
        const event = useRefEvent({
                mount() {}, // do something when component did mount
                clean() {}, // do something when component did unmount
                click() {}, // do something when component will be clicked
        })
        event('click', () => set((p) => p + 1)) // register new function
        return (
                <button ref={event.ref} onClick={event.click} className="clean-btn button button--secondary margin-left--md">
                        {i}
                </button>
        )
}

<App3 />

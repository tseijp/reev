...
export type ExtractArgs<T extends object> = T[keyof T] extends (...args: infer A) => any ? A : any[]

export interface DurableState<T extends object, R = unknown, Args extends unknown[] = ExtractArgs<T>> {
        <K = keyof T>(key?: K, ...args: Args): R
        (target?: T, ext?: T[keyof T], ...args: Args): R
}

export type MutableState<T extends object, Args extends unknown[] = ExtractArgs<T>> = T & {
...
export type EventState<T extends object, Args extends unknown[] = ExtractArgs<T>> = T & {
        (key?: keyof T, fun?: T[keyof T]): EventState<T, Args>
        (target?: Partial<T>): EventState<T, Args>
}

export type NestedFun<Ret, Args extends any[] = any[]> = Fun<[key: string, ...args: Args], Ret>

export type DurableFun<T extends object, Args extends unknown[] = ExtractArgs<T> & any> = Fun<[key: string, ...args: Args]>

export type EventFun<T extends object, Args extends unknown[] = ExtractArgs<T>> = Fun<[...args: Args], EventState<T>>

export type Fun<Args extends unknown[] = unknown[], Ret extends unknown = unknown> = (...args: Args) => Ret

 ...
 */
export const touchDistanceAngle = (
	event: TouchEvent,
	ids: number[]
): DistanceAngle | null => {
 ...
 */
export const pointerDistanceAngle = (
	pointerEvents: Map<number, PointerEvent>
): DistanceAngle | null => {
...
export const getCurrentTargetTouchIds = (event: TouchEvent): number[] => {
	return Array.from(event.touches)
		.filter(
			(touch) =>
				touch.target === event.currentTarget ||
				(event.currentTarget as Node)?.contains?.(touch.target as Node)
		)

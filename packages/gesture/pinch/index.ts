...
import {
	pinchDevice,
	touchDistanceAngle,
	pointerDistanceAngle,
	getCurrentTargetTouchIds,
	wheelPinchDelta,
	WebKitGestureEvent,
	DistanceAngle,
} from './utils'
...
export const pinchEvent = <El extends Element = Element>(
	...
	const pinching = (
		...
		self.delta[0] = self._value[0] !== 0
			? self.value[0] / self._value[0] - 1
			: 0
		...
		// Update scale
		self.scale = self.initial[0] !== 0
			? self.value[0] / self.initial[0]
			: 1
	...
	const pointerStart = (e: PointerEvent) => {
		...
		if (self.active) {
			...
			if (Array.from(_pointerEvents.keys()).every((id) =>
				self._pointerEvents.has(id)
			)) return
		}
